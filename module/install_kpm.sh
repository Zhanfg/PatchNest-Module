...
read_optional_prop() {
    ...
    _rc=$?
    case "$_rc" in
        0) OPTIONAL_PROP_VALUE=$_value; return 0 ;;
        1) return 0 ;;
        2) return 2 ;;
        *) return 3 ;;
    esac
...
load_optional_prop "$PROP_FILE" name
MOD_NAME=$OPTIONAL_PROP_VALUE
load_optional_prop "$PROP_FILE" version
MOD_VERSION=$OPTIONAL_PROP_VALUE
load_optional_prop "$PROP_FILE" event
MOD_EVENT=$OPTIONAL_PROP_VALUE
load_optional_prop "$PROP_FILE" args
MOD_ARGS=$OPTIONAL_PROP_VALUE
load_optional_prop "$PROP_FILE" autoLoad
MOD_AUTOLOAD=$OPTIONAL_PROP_VALUE
...
_signature_count=0
for _signature_candidate in "${KPM_FILE}.sig" "${KPM_FILE%.kpm}.sig"; do
    [ -f "$_signature_candidate" ] || continue
    _signature_count=$((_signature_count + 1))
    SIGNATURE_FILE=$_signature_candidate
done
[ "$_signature_count" -le 1 ] || fail "ZIP contains multiple signature files for the KPM"
_all_signature_count=$(find "$TMPDIR/root" -type f -name '*.sig' | wc -l | tr -d ' ')
[ "$_all_signature_count" = "$_signature_count" ] \
    || fail "ZIP contains a signature file that does not match the KPM"
...
if [ "$MOD_AUTOLOAD" = "true" ]; then
    ...
    if [ "$HAD_EXISTING" = "true" ]; then
        ...
        log "Signed KPM installed; reboot required before loading: $MOD_NAME ($MOD_ID) $MOD_VERSION"
    fi
