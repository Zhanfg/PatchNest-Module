...
# Run the read-only Android compatibility probe immediately before a direct
# write. This gate executes before blockdev --setrw and dd. It cannot be skipped
# by WebUI callers; test fixtures may replace the probe through the documented
# module-directory override.
runtime_compatibility_gate() {
  [ "${PATCHNEST_RUNTIME_COMPAT_ACTIVE:-0}" != "1" ] || return 0

  _module_dir=${PATCHNEST_MODDIR_OVERRIDE:-}
  if [ -z "$_module_dir" ]; then
    case "${MODPATH:-}" in
      */patch) _module_dir=${MODPATH%/patch} ;;
      *) echo "! Cannot resolve PatchNest module directory for compatibility check" >&2; return 1 ;;
    esac
  fi
  _probe="$_module_dir/runtime_compat_check.sh"
  [ -f "$_probe" ] && [ ! -L "$_probe" ] || {
    echo "! Runtime compatibility probe is missing or unsafe: $_probe" >&2
    return 1
  }

  _shell=/system/bin/sh
  if [ ! -x "$_shell" ]; then
    _shell=$(command -v sh 2>/dev/null || true)
  fi
  [ -n "$_shell" ] && [ -x "$_shell" ] || {
    echo "! No executable shell is available for runtime compatibility check" >&2
    return 1
  }

  PATCHNEST_RUNTIME_COMPAT_ACTIVE=1 \
  PATCHNEST_MODDIR_OVERRIDE="$_module_dir" \
  PATCHNEST_STATE_DIR="${PATCHNEST_STATE_DIR:-/data/adb/patchnest}" \
    "$_shell" "$_probe" --strict
}
...
flash_image() {
  ...
  runtime_compatibility_gate || {
    echo "! Runtime compatibility gate failed; no block write was attempted" >&2
    return 8
  }
