...
# This file is sourced by patch, unpatch, restore, post-fs-data, and status paths.
...
patchnest_resume_recovery_monitoring() {
  ...
  mkdir -p "$PATCHNEST_STATE_DIR" || return 1
  rm -f "$PATCHNEST_SUSPEND_FILE" "$PATCHNEST_RECOVERY_MARKER" "$PATCHNEST_RECOVERY_REQUEST" 2>/dev/null || return 1
  printf '%s\n' 0 >"$PATCHNEST_BOOT_COUNT" || return 1
  _pn_at=$(date -u +%Y-%m-%dT%H:%M:%SZ 2>/dev/null || true)
  _pn_state_tmp="${PATCHNEST_RECOVERY_STATE}.tmp.$$"
  cat >"$_pn_state_tmp" <<EOF
{
