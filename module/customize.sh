...
for _script in \
    ...
    runtime_compat_check.sh post-fs-data.sh service.sh status.sh \
...
ui_print "  4. Run runtime_compat_check.sh --strict before direct flashing"
ui_print "  5. Review the detected boot target before patching"
ui_print "  6. Reboot only after patch and readback verification succeeds"
