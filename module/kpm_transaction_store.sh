...
# directory. On any sidecar/manifest/checksum failure, all moved files are
# restored to their original locations before the function returns failure.
patchnest_store_kpm_transaction() {
  ...
  if [ "$_pt_failed" = "true" ]; then
    patchnest_restore_moved_sidecar "$_pt_entry/module.kpm.sig" "$_pt_sig_source" || true
    patchnest_restore_moved_sidecar "$_pt_entry/events" "$_pt_events_source" || true
    patchnest_restore_moved_sidecar "$_pt_entry/args" "$_pt_args_source" || true
    patchnest_restore_moved_sidecar "$_pt_entry/autoload" "$_pt_autoload_source" || true
    mv "$_pt_entry/$_pt_primary_name" "$_pt_source" 2>/dev/null || true
    rm -rf "$_pt_entry"
    return 1
  fi
...
EOF
  ...
  if ! mv "$_pt_manifest_tmp" "$_pt_entry/manifest.properties"; then
    _pt_failed=true
  fi
  ...
  printf '%s\n' 'state=complete' >"$_pt_entry/state"
