#!/system/bin/sh
# PatchNest KPM quarantine and failed-transaction inspector.
#
# Quarantine transactions may be listed, inspected, and strictly activated.
# Failed transactions are evidence-only: they may be listed and inspected but
# cannot be activated or deleted through this command.
...
usage() {
  cat <<'EOF'
Usage:
  manage_kpm_quarantine.sh list [quarantine|failed|all]
  manage_kpm_quarantine.sh inspect <entry-id>
  manage_kpm_quarantine.sh inspect <quarantine|failed> <entry-id>
  manage_kpm_quarantine.sh activate <entry-id>

Plain list/inspect remain backward-compatible and default to quarantine.
activate requires a complete checksum set, parseable KPM, and valid signature.
Failed transactions cannot be activated; they are read-only recovery evidence.
There is no force-overwrite or delete command.
EOF
}
...
scope_root() {
  case "$1" in
    quarantine) printf '%s' "$QUARANTINE_DIR" ;;
    failed) printf '%s' "$FAILED_DIR" ;;
    *) return 1 ;;
  esac
}
...
resolve_existing_entry() {
  _root=$1
  _requested=$(safe_entry_id "$2") || return 1
  case "$_root" in "$QUARANTINE_DIR"|"$FAILED_DIR") ;; *) return 1 ;; esac
  [ -d "$_root" ] && [ ! -L "$_root" ] || return 1
  _entry="$_root/$_requested"
  [ -d "$_entry" ] && [ ! -L "$_entry" ] || return 1
  printf '%s' "$_entry"
}

# Strict resolver used for checksum verification and activation. Incomplete or
# rollback-failed entries remain inspectable through resolve_existing_entry but
# can never cross this complete-transaction boundary.
resolve_entry() {
  _root=$1
  _requested=$2
  _entry=$(resolve_existing_entry "$_root" "$_requested") || return 1
  _manifest="$_entry/manifest.properties"
  _state="$_entry/state"
  [ -f "$_manifest" ] && [ ! -L "$_manifest" ] || return 1
  [ -f "$_state" ] && [ ! -L "$_state" ] || return 1
  [ "$(cat "$_state" 2>/dev/null)" = "state=complete" ] || return 1
...
list_scope() {
  _scope=$1
  _root=$(scope_root "$_scope") || fail "Unknown transaction scope: $_scope"
  [ -e "$_root" ] || return 0
  [ -d "$_root" ] && [ ! -L "$_root" ] || fail "Unsafe transaction root: $_root"

  for _entry in "$_root"/*; do
    [ -d "$_entry" ] && [ ! -L "$_entry" ] || continue
    _path_id=$(basename "$_entry")
    _state=$(cat "$_entry/state" 2>/dev/null || printf 'state=missing')
    _state=${_state#state=}
    _manifest="$_entry/manifest.properties"

    if ! _complete=$(resolve_entry "$_root" "$_path_id" 2>/dev/null); then
      printf '%s\t%s\t-\t-\t-\t-\t%s\n' \
        "$_scope" "$_path_id" "${_state:-invalid}"
      continue
    fi
    if ! verify_entry_checksums "$_complete"; then
      printf '%s\t%s\t-\tintegrity-failed\t-\t-\tfailed\n' \
        "$_scope" "$_path_id"
      continue
    fi
    ...
    printf '%s\t%s\t%s\t%s\t%s\t%s\tok\n' \
      "$_scope" "$_entry_id" "$_module_id" "$_reason" "$_created_at" "$_signed"
  done
}

list_entries() {
  _scope=${1:-quarantine}
  printf 'scope\tentry_id\tmodule_id\treason\tcreated_at\tsigned\tintegrity\n'
  case "$_scope" in
    quarantine) list_scope quarantine ;;
    failed) list_scope failed ;;
    all)
      list_scope quarantine
      list_scope failed
      ;;
    *) fail "Unknown transaction scope: $_scope" ;;
  esac
}
...
inspect_scope_entry() {
  ...
  _entry_id=$2
  _root=$(scope_root "$_scope") || fail "Unknown transaction scope: $_scope"
  _entry=$(resolve_existing_entry "$_root" "$_entry_id") \
    || fail "Invalid transaction entry: $_scope/$_entry_id"

  echo "scope=$_scope"
  echo "entry_id=$_entry_id"
  _state=$(cat "$_entry/state" 2>/dev/null || printf 'state=missing')
  echo "$_state"
  if resolve_entry "$_root" "$_entry_id" >/dev/null 2>&1 \
     && verify_entry_checksums "$_entry"; then
    echo "integrity=ok"
  else
    echo "integrity=failed-or-incomplete"
  fi

  if [ -f "$_entry/manifest.properties" ] && [ ! -L "$_entry/manifest.properties" ]; then
    cat "$_entry/manifest.properties"
  else
    echo "manifest=missing-or-unsafe"
  fi
  echo "files:"
  for _file in "$_entry"/*; do
    ...
    printf '  %s size=%s sha256=%s\n' "$_name" "$_size" "${_sha:-unavailable}"
  done
}

activate_entry() {
  # Activation is deliberately hard-bound to quarantine. Caller-selected scope
  # data is never accepted here; failed transactions cannot be activated.
  _original=$(resolve_entry "$QUARANTINE_DIR" "$1") \
    || fail "Invalid or incomplete quarantine transaction: $1"
  verify_entry_checksums "$_original" \
    || fail "Quarantine transaction checksum verification failed"
  ...
  for _source in "$_original"/*; do
    [ -f "$_source" ] && [ ! -L "$_source" ] \
      || fail "Transaction changed during activation"
    cp "$_source" "$_stage/$(basename "$_source")" \
      || fail "Cannot snapshot quarantine transaction"
  done
  verify_entry_checksums "$_stage" \
    || fail "Staged quarantine transaction checksum verification failed"

  _manifest="$_stage/manifest.properties"
  _module_id=$(read_field "$_manifest" module_id 2>/dev/null) \
    || fail "Entry has no unique module_id"
...
case "${1:-}" in
  ...
  inspect)
    case "$#" in
      2) inspect_scope_entry quarantine "$2" ;;
      3) inspect_scope_entry "$2" "$3" ;;
      *) usage >&2; exit 2 ;;
    esac
    ;;
  activate)
