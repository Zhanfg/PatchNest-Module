...
printf '%s\n' '[1/7] Parse transaction shell sources'
bash -n \
...
printf '%s\n' '[2/7] Compile transaction contract tests'
python3 -m py_compile \
  tests/validate_kpm_package_metadata.py \
  tests/validate_transaction_store.py \
  tests/validate_transaction_visibility.py \
  tests/validate_install_recovery.py

printf '%s\n' '[3/7] Validate KPM package metadata contracts'
python3 tests/validate_kpm_package_metadata.py

printf '%s\n' '[4/7] Validate transaction store contracts'
python3 tests/validate_transaction_store.py

printf '%s\n' '[5/7] Validate transaction visibility contracts'
python3 tests/validate_transaction_visibility.py

printf '%s\n' '[6/7] Run transaction rollback vectors'
bash tests/test_kpm_transaction_store.sh

printf '%s\n' '[7/7] Run durable install recovery vectors'
