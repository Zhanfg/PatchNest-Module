...
The scanner requires no network, root access, Android device, or GitHub Actions
runner. By default it only writes reports. ``--enforce`` makes blockers and
...
EXCLUDED_DIRS = {".git", "node_modules", "dist", "build", "out", ".venv", "venv", "__pycache__"}
...
def check_boot_partition_selection(root: Path, findings: list[Finding]) -> None:
    ...
    if not path.exists():
        findings.append(Finding("blocker", "BOOT-001", str(path.relative_to(root)), None, "boot helper is missing"))
        return
    text = read_text(path)
    parsed = function_body(text, "find_boot_image")
    if not parsed:
        findings.append(Finding("blocker", "BOOT-002", str(path.relative_to(root)), None, "find_boot_image() was not found"))
        return
    body, start = parsed
...
def check_flash_contract(root: Path, findings: list[Finding]) -> None:
    ...
    text = read_text(path)
    required = {
        "FLASH-001": ("verify_block_image_prefix", "block-device writes have no dedicated readback verifier"),
    ...
    parsed = function_body(text, "flash_image")
    if parsed and "Not block or char device, storing image" in parsed[0]:
        findings.append(
            Finding(
                "error",
                "FLASH-005",
                str(path.relative_to(root)),
                line_number(text, parsed[1]),
                "flash_image() accepts regular files; direct-flash callers must reject non-device targets before reporting success.",
...
def check_backup_contract(root: Path, findings: list[Finding]) -> None:
    ...
    if patch_path.exists():
        ...
        required = {
            "BACKUP-002": ("backup_sha256", "backup manifest does not persist a backup digest"),
            "BACKUP-003": ("backup_verified", "backup manifest does not persist verification state"),
            "BACKUP-004": ("magiskboot unpack", "backup or repacked image is not unpack-tested"),
        }
        for check_id, (token, message) in required.items():
            if token not in text:
    ...
    if unpatch_path.exists():
        ...
        required = {
            ...
            "RECOVERY-002": ("backup_sha256", "recovery does not validate the stored backup digest"),
            "RECOVERY-003": ("boot_image", "recovery is not bound to a recorded target partition"),
        }
        for check_id, (token, message) in required.items():
            if token not in text:
...
def check_shell_hazards(root: Path, findings: list[Finding]) -> None:
    for path in iter_text_files(root):
        ...
        add_regex_findings(
            ...
            severity="warning",
            check_id="SHELL-001",
            message="root shell path contains eval; verify both variable name and value are constrained",
        )
        add_regex_findings(
            findings,
            root=root,
            ...
            if unescaped:
                findings.append(
                    Finding(
                        ...
                        str(path.relative_to(root)),
                        line_number(text, match.start()),
                        "privileged exec() template contains interpolation not visibly wrapped by escapeShell(); verify it is constant or validated",
                        " ".join(unescaped)[:240],
                    )
                )
...
def deduplicate(findings: Iterable[Finding]) -> list[Finding]:
    ...
    for finding in findings:
        key = (finding.severity, finding.check_id, finding.path, finding.line, finding.message, finding.evidence)
        unique[key] = finding
    return sorted(
        unique.values(),
...
def render_markdown(root: Path, findings: Sequence[Finding]) -> str:
    ...
    for finding in findings:
        ...
        if finding.evidence:
            escaped_evidence = finding.evidence.replace("`", "\\`")
            lines.append(f"- Evidence: `{escaped_evidence}`")
        lines.append("")
    return "\n".join(lines) + "\n"
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    markdown_path = output_dir / "offline-audit.md"
    json_path.write_text(json.dumps(payload, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")
    markdown_path.write_text(render_markdown(root, final), encoding="utf-8")
